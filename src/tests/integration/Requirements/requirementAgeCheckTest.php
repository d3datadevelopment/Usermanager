...
class requirementAgeCheckTest extends d3RequirementIntegrationTestCase
{
    ...
    public function createTestData()
    {
        ...
        $this->createUser(
            ...
            array(
                'oxcompany'     => __CLASS__,
                'oxbirthdate'   => date('Y-m-d', strtotime("-39 year", time())),
            )
        );
        ...
        $this->createUser(
            ...
            array(
                'oxcompany' => __CLASS__,
                'oxbirthdate'   => date('Y-m-d', strtotime("-40 year", time())),
            )
        );
        ...
        $this->createUser(
            ...
            array(
                'oxcompany' => __CLASS__,
                'oxbirthdate'   => date('Y-m-d', strtotime("-41 year", time())),
            )
        );
     ...
     * @throws Exception
     */
    public function getConfiguredManagerLess()
    {
        $oManager = $this->getManagerMock($this->sManagerId);

        $oManager->setValue('blCheckAgeCheck_status', true);
        $oManager->setValue('sAgeCheckType', 'less');
        $oManager->setValue('sAgeCheckValue', '40');
    ...
    public function getConfiguredManagerHigher()
    {
        ...
        $oManager->setValue('blCheckAgeCheck_status', true);
        $oManager->setValue('sAgeCheckType', 'higher');
        $oManager->setValue('sAgeCheckValue', '40');
